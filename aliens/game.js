...
function myGameState() {
  ...
  var health_sprite_sheet = new jaws.SpriteSheet({image: "health.png", frame_size: [80,10] })
  var health_sprite = new jaws.Sprite({image: "health.png",x:10, y:5, anchor: "top_left"})
  var life_sprite = new jaws.Sprite({image: "lifes.png",x:0, y:5, anchor: "top_left"})
  var exp_anim = new jaws.Animation({sprite_sheet: 'exp.png', frame_size: [30,30], frame_duration: 50});
  exp_anim.bounce = true;
  var anim = new jaws.Animation({sprite_sheet: 'ship.png', frame_size: [80,30], frame_duration: 50});
  anim.bounce = true;
  ...
  function new_ship(){
    ...
    Ship.init = function(){
      ...
      this.speed = 5;
      this.shield = 0;
      this.shield_sprite = new jaws.Sprite({image: "shield_sprite.png",x:10, y:10, anchor: "top_left"})
    }
    Ship.init()
    Ship.lifes = 3;
    Ship.points = 0;
  ...
  function new_static(image,x,y,xspeed,yspeed){
    ...
    var shield = function (ship){ship.shield = 1;setTimeout(function(ship){ship.shield=0},10000,ship)}
    var gifts_list = [
    ['care.png',care],
    ['life.png',life],
    ['due.png',due],
	...
	case 2: return Math.round(500+Math.random()*(100000*(1/total_enemies))); break;
      }
    }
    var create_enemy
    var enemies_types = [
	...
	var enemy = new_static("enemy"+enemies_types[type][0]+'.png',jaws.width,py,-2,0);
	...
	var bullet = new_static("bullet.png", ship.x+ship.width, ship.y+ship.height/2+d_y[i], 15, 0);
	bullets.push(bullet);
      }
	...
	{
	  var bullet = new_static("bullet.png", ship.x+ship.width, ship.y+ship.height/2, 15, ys[i]);
	  bullets.push(bullet);}
      }
    }
    ...
    this.setup = function() {
      ...
      jaws.on_keydown(["p"],  function()  {paused=!paused})
      player = new_ship()
	...
	if(jaws.pressed("down")) {
	  if (player.y+player.speed<jaws.height-player.height) {player.y += player.speed;}}
	  if(jaws.pressed("up")) {
	    if (player.y-player.speed>0){player.y -= player.speed;}}
	    player.shield_sprite.y = player.y;
	    if(jaws.pressed("space") && player.can_fire) {
	      player.can_fire=0;
	    ...
	    var del = function(item){enemies.remove(item)}
		...
		if (jaws.isOutsideCanvas(item)){
	      ...
	      bullets.update();
	      enemies.forEach(function (element, index, array){if(element.health<1){element.setImage(element.exp_anim.next() );element.health-=1;}})
	      missiles.deleteIf(jaws.isOutsideCanvas);
	      enemies.update();
	      var col=jaws.collideManyWithMany(bullets,enemies);
	      for (var i=0;i<col.length;i++)
	      {
		if(col[i][1].health>0){bullets.remove(col[i][0])}
		col[i][1].health -= 1
		if(col[i][1].health==0){player.points+=1;setTimeout(del, 600, col[i][1])}
		if(col[i][1].health<1){col[i][1].xspeed = 0}
	      }
	      ...
	      //d.innerHTML = jaws.game_loop.ticks+' '+player.ticks
      }
    }
	...
	life_sprite.draw()}
	jaws.context.fillText("Aliens to invasion: "+(10-player.aliens), 100+15*player.lifes+w.width,15)
	if (paused){jaws.context.font = "bold 20pt terminal";
	  jaws.context.lineWidth = 10
	  jaws.context.fillStyle =  "Black"
...
function MenuState() {
  this.setup = function() {
    jaws.on_keydown(["enter","space"],  function()  {jaws.switchGameState(myGameState)} )
    x = 50
    delta = 1 
  }
  ...
  this.draw = function() {
    ...
    if (localStorage.getItem("record")!=null){
      jaws.context.fillStyle =  "Black"
      jaws.context.fillText("Record: "+localStorage.getItem("record"), 30, jaws.height/2-30)
    }
    jaws.context.fillStyle =  "rgb("+x+","+x+","+x+")"
    jaws.context.fillText("Press the spacebar to start", 30, jaws.height/2)
  }
}
...
function GameOverState() {
  ...
  this.setup = function() {
    ...
    if (parseInt(localStorage.getItem("record"))<last_points){new_record = true}
  ...
  this.draw = function() {
    ...
    jaws.context.fillText("You scored "+last_points+" points", 30, jaws.height/2-60)
    if (new_record){
